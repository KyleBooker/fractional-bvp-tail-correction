/* first_order_first_derivative.c — solver variant with tail scheme (7b)
 *
 *   u(x_{j+1}) = u(x_j) * (1 - (gamma+1) / (j - N/2))
 *
 * First-order forward difference discretisation of the asymptotic
 * relation u' ~ -(gamma+1) u / y. See §1.1, equation (7b) of Booker &
 * Nec (2019). Differs from the other three sibling files only in the
 * "decay based on asymptotics" loop near the end of solve_numerical_system().
 *
