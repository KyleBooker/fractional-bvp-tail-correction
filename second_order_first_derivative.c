/* second_order_first_derivative.c — solver variant with tail scheme (7c)
 *
 *   u(x_{j+2}) = (2 u(x_{j+1}) - u(x_j)/2) / (3/2 + (gamma+1)/(j - N/2))
 *
 * Second-order backward difference discretisation of the first-derivative
 * asymptotic relation u' ~ -(gamma+1) u / y. See §1.1, equation (7c) of
 * Booker & Nec (2019). Differs from the other three sibling files only in
 * the "decay based on asymptotics" loop near the end of
