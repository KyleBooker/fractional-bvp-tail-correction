/* second_order_second_derivative.c — solver variant with tail scheme (9c)
 *
 *   u(x_{j+2}) = ( u(x_{j+1}) (2 - B/(j-N/2)^2)
 *               -  u(x_j)     (1 - A/(2j-N)) )
 *               / ( 1 + A/(2j-N) ),
 *   with B = (gamma+1)(A - gamma - 2).
 *
 * Second-order central-difference discretisation of the second-derivative
 * tail relation u'' + A u'/y + B u/y^2 ~ 0. See §1.2, equation (9c) of
 * Booker & Nec (2019). Of the four schemes this one was found to give
 * the most stable results across the (p, gamma) parameter space and is
 * the variant inlined in the main homoclinic.c. Differs from the other
 * three sibling files only in the "decay based on asymptotics" loop
 * near the end of solve_numerical_system().
 *
 * Builds identically to homoclinic.c (same input, same output filenames).
 */
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
 ...
 for(m=1;m<=mt;m++){
   ...
   for(j=i-4;j<N-2;j++){
     if(g==2.0)
       ...
       *(U+j+3)=(*(U+j+2))*(2.5 - (0.75*A)/(j-N/2))-(*(U+j+1))*(2.0 - A/(j-N/2)) + (*(U+j))*(0.5 - (0.25*A)/(j-N/2) - (0.5*B)/((j-N/2)*(j-N/2))); // KYLE'S CHANGE 2nd order
   }
