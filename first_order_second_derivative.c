/* first_order_second_derivative.c — solver variant with a first-order
 * backward-difference form of the second-derivative tail relation
 *
 *   u'' + A u'/y + B u/y^2 ~ 0,   B = (gamma+1)(A - gamma - 2),
 *
 * discretised here as
 *
 *   u(x_{j+2}) = (2 u(x_{j+1}) - u(x_j)/2) / (3/2 + (gamma+1)/(j-N/2)).
 *
 * See §1.2 of Booker & Nec (2019). Differs from the other three sibling
 * files only in the "decay based on asymptotics" loop near the end of
...
// for(g=1.0;g<=2.0;g+=0.01){ // block for a single homoclinic computation
 ...
 for(m=1;m<=mt;m++){
   ...
   for(j=i-4;j<N-2;j++){
      ...
      *(U+j+2)=((*(U+j+1))*2.0 - (*(U+j))*0.5 )/(1.5 + ((g+1.0)/(j-N/2)));
   }   
